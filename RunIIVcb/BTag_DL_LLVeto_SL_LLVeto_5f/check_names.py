...
import object_corrections.utils as utils
...
class MissingCMSSystDatabase(Exception):
    pass
...
parser.add_argument('--uncertainty-sets', nargs='*', help='either a list of uncertainty sets which should be included or a text file with a list of uncertainty sets')
parser.add_argument('--syst-db', default='object_corrections/object_uncertainties.json', help='path to cms systematics database')
...
if args.make_template is not None:
    ...
    if args.uncertainty_sets:
        if len(args.uncertainty_sets) == 1 and os.path.exists(args.uncertainty_sets[0]):
            with open(args.uncertainty_sets[0], 'r') as f:
                uncertainty_sets = [line.rstrip() for line in f]
        else:
            uncertainty_sets = args.uncertainty_sets

        if os.path.exists(args.syst_db):
            db = utils.CMSSystDatabase(args.syst_db)
        else:
            raise MissingCMSSystDatabase(f'CMS Systematics database expected to be found at {args.syst_db} but does not exist.') 

        for unc_set in uncertainty_sets:
            if not unc_set in db.list('uncertainty_set'):
                raise ValueError(f'Error: No existing uncertainty set named {unc_set} was found in the database')

        db.select('uncertainty_set', uncertainty_sets, regex=False)
        combine_names = db.list(['combine_name','uncertainty_set'])

        systs_from_set = {}
        for rgx, info in input_master.items():
                all_names = utils.expand_rgx( rgx )
                for comb_name, unc_set in combine_names.items():
                    if not len(unc_set) == 1:
                        raise ValueError(f'Found combine name {comb_name} which does not have a unique uncertainty set from those listed (found: {unc_set})')
                    if comb_name in all_names:
                        systs_from_set[ comb_name ] = {'description': info['description'], 'class': info['class']}

        missing_uncertainties = {}
        all_datacard_systs = set([])
        cb.ForEachSyst( lambda x: all_datacard_systs.add(x.name()) )
        for cmb_name, unc_set in combine_names.items():
            if not cmb_name in all_datacard_systs:
                if unc_set[0] in missing_uncertainties:
                    missing_uncertainties[unc_set[0]].append(cmb_name)
                else:
                    missing_uncertainties[unc_set[0]] = [ cmb_name ]
        if len(missing_uncertainties) > 0:
            n_missing = sum([ len(v) for _, v in missing_uncertainties.items() ])
            n_sets = len(missing_uncertainties.keys() )
            print(f'WARNING: There are {n_missing} uncertainties from {n_sets} uncertainty sets which were expected but not found in your datacard')
            print(f'The missing uncertainties are:')
            yaml.dump( missing_uncertainties, sys.stdout, indent=6)
        systematics_dict.update( systs_from_set )
