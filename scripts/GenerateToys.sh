...
combine --toysFrequentist --bypassFrequentistFit -t 5 --saveToys --setParameters "r=${injection},rgx{mask_Signal_.*}=0" ${freeze_opt} -M GenerateOnly -s $seed -d morphedWorkspace_fitDiagnostics120.root -n .Injec${injection_str} --rMin $rmin --rMax $rmax --expectSignal ${injection}
